...
public class Octree{
    ...
    private void returningAux(Node root) {
        ...
        if(root.bees==null || root.bees.size()==0) {
          ...
          if(root.father!=null){
            Point3D bee1= root.bees.getFirst();
            for(Node child :root.father.children){
              for(Point3D bee2 : child.bees){
                double near= Math.sqrt(Math.pow(bee1.getX()-bee2.getX(), 2)+Math.pow(bee1.getY()-bee2.getY(), 2)+Math.pow(bee1.getZ()-bee2.getZ(), 2));
                if(near<=100 && near!=0)
