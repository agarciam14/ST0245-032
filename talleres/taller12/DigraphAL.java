 ...
 *
 * @author Mauricio Toro, Mateo Agudelo, Anthony García, Daniel Hincapié
 */
public class DigraphAL extends Digraph{
    ...
    public ArrayList<Integer> getSuccessors(int vertex) {
        ...
        for(int i=0; i<grafoConListas.get(vertex).size();i++){
        Pair<Integer,Integer> p1=grafoConListas.get(vertex).get(i);
        if((p1.getValue())!=0){
            int key=p1.getKey();
            respuesta.add(key);
        }
    ...
    public int getWeight(int source, int destination){
        Pair<Integer,Integer> p1=grafoConListas.get(source).get(destination);
        return p1.getValue();
    }
